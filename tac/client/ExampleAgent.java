...
/**
 * The <code>ExampleAgent</code> is an example of a simple Manufacturer agent
 * using <code>{@link SCMAgent}</code> to simplify the implementation.
 * <p>
 * 
 * This example manufacturer uses strict build to order. In optimal case be
 * possible to deliver in 6 days (including one day for the suppliers to produce
 * the supply).
 * <p>
 * 
 * <dl>
 ...
 * </dl>
 * <p>
 * This means that the example manufacturer will never bid for requests with a
 * too short due date.
 * <p>
 * 
 ...
 * <li>Delivers to customers on due date (customers will not pay earlier anyway)
 * <li>Removes too late orders when the customer cancels them and reuses the
 * components/products for other orders.
...
public class ExampleAgent extends SCMAgent {
	...
	/**
	 * Called when the agent received all startup information and it is time to
	 * start participating in the simulation.
	 */
	...
	/**
	 * Called when a game/simulation has ended and the agent should free its
	 * resources.
	 */
	...
	/**
	 * Called when a bundle of RFQs have been received from the customers. In
	 * TAC03 SCM the customers only send one bundle per day and the same RFQs
	 * are sent to all manufacturers.
	 * 
	 * @param rfqBundle
	 *            a bundle of RFQs
	 */
	...
	/**
	 * Called when a bundle of orders have been received from the customers. In
	 * TAC03 SCM the customers only send one order bundle per day as response to
	 * offers (and only if they want to order something).
	 * 
	 * @param newOrders
	 *            the new customer orders
	 */
	...
	/**
	 * Called when a bundle of offers have been received from a supplier. In
	 * TAC03 SCM suppliers only send on offer bundle per day in reply to RFQs
	 * (and only if they had something to offer).
	 * 
	 * @param supplierAddress
	 *            the supplier that sent the offers
	 * @param offers
	 *            a bundle of offers
	 */
	...
	/**
	 * Called when a simulation status has been received and that all messages
	 * from the server this day have been received. The next message will be for
	 * the next day.
	 * 
	 * @param status
	 *            a simulation status
	...
	protected synchronized void handleSimulationStatus(SimulationStatus status) {
		...
		if (orders != null) {
			for (int i = 0, n = orders.length; i < n; i++) {
				...
				int inventoryQuantity = inventory
				...
				} else if (addProductionRequest(productID, orderedQuantity
						- inventoryQuantity)) {
	...
	private void cancelCustomerOrder(Order order) {
		...
		int[] components = getBOMBundle().getComponentsForProductID(
				order.getProductID());
		if (components != null) {
			int quantity = order.getQuantity();
			for (int j = 0, m = components.length; j < m; j++) {
